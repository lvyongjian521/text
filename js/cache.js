...
layui.use(['form','jquery',"layer"],function() {
    ...
    if(!/http(s*):\/\//.test(location.href)){
        layer.alert("请先将项目部署到 localhost 下再进行访问【建议通过tomcat、webstorm、hb等方式运行，不建议通过iis方式运行】，否则部分数据将无法显示(或者检测到你拷贝我网站源码，因此被阻止访问)");
    ...
    function showNotice(){
        layer.open({
            ...
            btn: ['火速围观'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">测试。</span></p></div>',
    ...
    function lockPage(){
        layer.open({
            ...
            content : '<div class="admin-header-lock" id="lock-box">'+
                            ...
                            '<div class="admin-header-lock-name" id="lockUserName">吕甬建</div>'+
                            '<div class="input_btn">'+
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".admin-header-lock-input").val() == ''){
            ...
            if($(this).siblings(".admin-header-lock-input").val() == "lyj312325824"){
